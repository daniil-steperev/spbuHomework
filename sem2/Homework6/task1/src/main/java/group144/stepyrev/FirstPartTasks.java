...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(album -> album.getName(), Collectors.counting()))
                .values().stream()
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        ...
        .map(Track::getRating)
        .max(Comparator.comparing(track -> track)).orElse(0)));
    }

    // Список альбомов, отсортированный по убыванию среднего рейтинга его треков (0, если треков нет)
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(
                Comparator.comparing(album -> (-1) * album.getTracks().stream().mapToInt(Track::getRating)
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce((first, second) -> (first * second) % modulo).orElse(0);
    }
