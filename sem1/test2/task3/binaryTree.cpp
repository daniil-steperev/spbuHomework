...
void addToTree(BinaryTree *tree, int element)
{
    if (isEmpty(tree))
    {
        tree->root = new Node {element, 1, nullptr, nullptr};
        return;
    }
    if (element == tree->root->value)
    {
...
void writeFile(ofstream &fileOut, Node *son)
{
    if (son == nullptr)
    {
        return;
    }
    writeFile(fileOut, son->leftChild);
    fileOut << son->value << ' ';
    writeFile(fileOut, son->rightChild);
}

void writeToFile(ofstream &fileOut, BinaryTree *tree)
{
    writeFile(fileOut, tree->root);
}
